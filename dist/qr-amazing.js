/* Avtomatik yig'ilgan fayl — tahrirlamang. Manba: src/*.js, yig'uvchi: build.mjs */
/* ===== src/vc.js ===== */
/**
...
function weightVectors(n, parity) {
  const out = [];
  for (let v = 0; v < 1 << n; v++) {
    let bits = 0;
 ...
 */
function encodeShares(bin, w, h, n, rng = Math.random) {
...
function stackShares(shares, width, height) {
 ...
 */
function simulateStack(shares, geom) {
 ...
 */
function adjust(lum, { brightness = 0, contrast = 0, gamma = 1, invert = false, autoLevels = false } = {}) {
 ...
 */
function dither(lum, w, h, method = 'fs', threshold = 0.5) {
...
function prepareBinary(rgba, sw, sh, pixelW, pixelH, opts = {}) {
 ...
 */
function fitPixelGrid(maxW, maxH, ratio) {
  const cands = [];
 ...
 */
function buildSheetDoc({ shares, moduleCols, moduleRows, layout, meta = {} }) {
  const pages = [];
